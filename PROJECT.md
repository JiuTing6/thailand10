...
Tag 系统由 **filter 一处统一打 tag**（topic + city 都在 filter 阶段决定，translate 只翻译）。任何增减都必须同步以下 **3 处**：

1. **`scripts/filter.py`** — `VALID_TOPICS` 集合（topic）+ `KNOWN_CITY_TAGS` 集合（city，仅供参考；模型可输出未在列表里的具体城市名）+ SYSTEM_PROMPT 里的 topic / city 段
2. **`newsroom.html`** — `KNOWN_CITIES` / `TOPIC_TAGS` 数组
3. **本文（PROJECT.md）** — 上面两个表

新增 city/topic 的标准操作：
- 新增 topic → 3 处全改 → 跑一次 ingest 看新 tag 是否被分配 → 历史 pool 不回补（向前生效）
- 新增 city（提升到"已知城市"） → 3 处全改
