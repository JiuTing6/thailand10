...
# 按 topic 的差异化入库门槛（缺省用 RELEVANCE_THRESHOLD）。
# #治安 提高到 0.7：pool 长期治安占比过高（~35%），琐碎犯罪/事故噪音多，
# 只保留高相关治安（涉外国人/华人、重大案件、影响出行安全的事件）。
TOPIC_THRESHOLDS = {
    "#治安": 0.7,
}
...
# 已知 6 个城市；其他城市按"#<具体城市名>"输出，由 newsroom 归入 #更多地区
KNOWN_CITY_TAGS = {"#曼谷", "#芭提雅", "#普吉岛", "#清迈", "#苏梅岛", "#泰国"}

SYSTEM_PROMPT = """你是泰国新闻分类过滤器。输入是一批新闻条目（JSON数组），你的任务是：
1. 判断每条新闻是否与泰国本地相关
2. 对相关新闻归入topic分类，并给出相关度评分

## 9个Topic定义（输出 topic_tag 字段，恰好1个）
- #时政：泰国政治/外交/地缘/政府决策
...
- `#苏梅岛`：明确属于苏梅岛
- `#泰国`：全国性新闻 / 无法确定具体城市
- 其他城市（华欣、清莱、孔敬、巴真等）：输出 `#<具体城市名>`，例如 `#华欣` `#清莱`
...
## #治安 类特别评分规则（治安新闻噪音多，从严打分）
- **0.7+ 才入库**，所以只有真正重要的治安事件才给 ≥0.7：
  - 涉及外国人/华人/游客的案件、诈骗、暴力
  - 重大刑案、跨境犯罪、大规模灾害事故
  - 影响出行/居住安全的系统性事件（如某区治安恶化、新型骗局预警）
- **琐碎治安给 0.4-0.6**（会被过滤掉）：单起普通车祸、邻里纠纷、与外国人无关的本地小案、天气对生意的影响、常规交通管理
...
## 输出格式
对每条输入，**只输出 4 个字段**（不要回显原始 title/desc/url 等任何字段，避免转义问题）：
- id: 输入条目的 id 字段（原样复制，用于匹配）
- topic_tag: 9个topic之一，不相关则填 null
- city_tag: 上述 city_tag 之一（不相关条目可填 null）
...
def main():
    ...
    for item in all_scored:
        ...
        # city_tag: 缺失/无效则兜底 #泰国（不让条目因 city 缺失被丢）
        ctag = item.get("city_tag")
